...
pub(crate) fn escape_string(string: &str) -> String {
    ...
    for chr in string.chars() {
        ...
        //
        // Local fix: apostrophes are legal inside quoted RC strings and must
        // not be escaped. Upstream 0.3.6 escapes them as `\'`, which breaks
        // absolute paths containing `'` (e.g. folder names like "Henry's_X")
        // with RC2135 file-not-found, and leaves a stray backslash in
        // VERSIONINFO metadata strings.
        match chr {
            '"' => escaped.push_str("\"\""),
            '\'' => escaped.push('\''),
            '\\' => escaped.push_str("\\\\"),
